    ...
    describe('rateBands', () => {
      ...
      it('has middle band', () => {
        expect(rateBands.middle).toEqual({
          percentage: 2, threshold: 7360
        });
      });

      it('has upper band', () => {
        expect(rateBands.upper).toEqual({
          percentage: 5, threshold: 50672
        });
      });
  ...
  describe('calculate()', () => {
    describe('weekly', () => {
      ...
      describe('when pay over the exemption threshold', () => {
        ...
        it('includes the calculation steps in results', () => {
          expect(results.steps).toEqual(['Calculate USC on lower threshold: 12012 X 0.5% = 60.06','Calculate USC on middle threshold: 988 X 2% = 19.76','Calculate USC on upper threshold: 0 X 5% = 0.00','Calculate USC on balance: 0 X 8% = 0.00','USC payable total: 79.82','USC payable per period: 79.82 / 52 = 1.54']);
        });
      });
    ...
    describe('monthly', () => {
      ...
      describe('when pay over the exemption threshold', () => {
        ...
        it('includes the calculation steps in results', () => {
          expect(results.steps).toEqual(['Calculate USC on lower threshold: 12012 X 0.5% = 60.06','Calculate USC on middle threshold: 988 X 2% = 19.76','Calculate USC on upper threshold: 0 X 5% = 0.00','Calculate USC on balance: 0 X 8% = 0.00','USC payable total: 79.82','USC payable per period: 79.82 / 12 = 6.65']);
        });
      });
